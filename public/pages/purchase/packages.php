...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php if ($trial_button_disabled && !empty($trial_expire_date)): ?>
    function updateTrialCountdown() {
        const expireDate = new Date('<?php echo $trial_expire_date; ?>');
        const now = new Date();
        ...
        if (timeDiff > 0) {
            ...
            if (countdownElement) {
                countdownElement.textContent = daysRemaining + ' ngày';
            }
        } else {
            // Trial restriction has expired, reload page to show trial option again
            location.reload();
        }
    }
    ...
    setInterval(updateTrialCountdown, 3600000); // Update every hour
    <?php endif; ?>
