                ...
                <?php foreach ($all_packages as $package): ?>                    <?php                        // --- LOGIC GÓI DÙNG THỬ ---
                    ...
                    <div class="<?php echo $card_classes; ?>">
                        ...
                        <button class="<?php echo $button_classes; ?> disabled" disabled>
                            ...
                            <span class="countdown" data-expire-date="<?php echo htmlspecialchars($trial_expire_date); ?>">(Còn <?php echo $trial_days_remaining; ?> ngày)</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Thêm chức năng countdown thời gian thực
    const countdownElements = document.querySelectorAll('.countdown[data-expire-date]');
    if (countdownElements.length > 0) {
        // Hàm cập nhật countdown
        function updateCountdown() {
            countdownElements.forEach(el => {
                const expireDate = new Date(el.getAttribute('data-expire-date')).getTime();
                ...
                if (timeLeft > 0) {
                    ...
                    // Hiển thị kết quả
                    if (days > 0) {
                        el.innerHTML = `(Còn ${days} ngày ${hours}h:${minutes}p:${seconds}s)`;
                    } else {
                        el.innerHTML = `(Còn ${hours}h:${minutes}p:${seconds}s)`;
                    }
                } else {
                    el.innerHTML = "(Đã hết thời gian)";
                }
            });
        }
        
        // Cập nhật countdown ngay lập tức và sau đó mỗi giây
        updateCountdown();
